...
//// 概要 : インスタンス生成時に行う処理
////=============================================================================
//CMotion::CMotion(const char * pFileName, CModel3D *pParent)
//{
//	// パーツ名の初期化
...
//	{
//		D3DXVECTOR3 pos = m_pParts[nCntParts]->GetPos();
//		D3DXVECTOR3 rot = m_pParts[nCntParts]->GetRot();
//		D3DXVECTOR3 posOrigin = m_pParts[nCntParts]->GetPosOrigin();
//		D3DXVECTOR3 rotOrigin = m_pParts[nCntParts]->GetRotOrigin();
//
//		// 位置と向きの初期値を保存
...
//
//			// 目的の位置と向きの算出
//			D3DXVECTOR3 posDest = (posOrigin + motion->pKeySet[motion->nCntKeySet].pKey[nCntParts].pos) - pos;
//			D3DXVECTOR3 rotDest = (rotOrigin + motion->pKeySet[motion->nCntKeySet].pKey[nCntParts].rot) - rot;
//
//			// 角度の正規化
...
//
//		// 変数宣言
//		D3DXVECTOR3 posDest = m_pParts[nCntParts]->GetPosDest();
//		D3DXVECTOR3 rotDest = m_pParts[nCntParts]->GetRotDest();
...
//			D3DXVECTOR3 posDest = posOrigin + myKey.pos - pos;
//			D3DXVECTOR3 rotDest = rotOrigin + myKey.rot - rot;
//
//			// 角度の正規化
//			rotDest.x = RotNormalization(rotDest.x);
//			rotDest.y = RotNormalization(rotDest.y);
...
//								{
//									if (strcmp(&m_partsFile[nType][0], pModelManager->GetFileName(nCntModel)) == 0)
//									{
//										m_pParts[nCntParts] = CParts::Create(nCntModel);
//										break;
//									}
//								}
//							}
...
//								{
//									m_pParts[nCntParts]->SetParent(m_pParent);
//								}
//								else
//								{
//									m_pParts[nCntParts]->SetParent(dynamic_cast<CModel3D*>(m_pParts[nIdxModelParent]));
//								}
//							}
...
void CMotion::SetMotion(const int nCntMotionSet)
{
	...
	for (int nCntParts = 0; nCntParts < m_nMaxParts; nCntParts++)
	{
		...
		// 角度の正規化
		parts->rot.x = CCalculation::RotNormalization(parts->rot.x);
		parts->rot.y = CCalculation::RotNormalization(parts->rot.y);
		parts->rot.z = CCalculation::RotNormalization(parts->rot.z);
	}
}
	...
	for (int nCntParts = 0; nCntParts < m_nMaxParts; nCntParts++)
	{
		...
		{// フレームカウントが0の時
		 // 目的の位置と向きの算出
			...
			// 角度の正規化
			parts->rotDest.x = CCalculation::RotNormalization(parts->rotDest.x);
			parts->rotDest.y = CCalculation::RotNormalization(parts->rotDest.y);
			parts->rotDest.z = CCalculation::RotNormalization(parts->rotDest.z);
		}
